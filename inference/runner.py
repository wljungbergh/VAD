...
class VADRunner:
    ...
    def forward_inference(self, input: VADInferenceInput) -> VADInferenceOutput:
        ...
        img_feats = self.extract_feat(img=imgs, img_metas=img_metas)

        outs = self.pts_bbox_head(
            img_feats,
            img_metas,
            prev_bev=self.prev_frame_info["prev_bev"],
            ego_his_trajs=None,  # figure these out
            ego_lcf_feat=None,  # figure these out
        )
        ...
        for i, (
            ...
            bbox_result["ego_fut_cmd"] = None  # ego_fut_cmd.cpu() | figure this out
            bbox_results.append(bbox_result)

        return VADInferenceOutput(
            trajectory=torch.zeros(6, 2).numpy(), aux_outputs=None
        )
