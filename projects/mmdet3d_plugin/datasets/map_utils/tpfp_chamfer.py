...
def vec_iou(pred_lines, gt_lines):
    ...
    pred_lines_shapely = [
        LineString(i).buffer(
            1.0, cap_style=CAP_STYLE.round, join_style=JOIN_STYLE.round
        )
        for i in pred_lines
    ]
    gt_lines_shapely = [
        ...
        for o in tree.query(pline):
            if o.intersects(pline):
                ...
                if metric == "POR":
                    ...
                    fdistance_1 = -similaritymeasures.frechet_dist(
                        pred_lines[pred_id], gt_lines[i]
                    )
                    fdistance_2 = -similaritymeasures.frechet_dist(
                        pred_lines[pred_id][::-1], gt_lines[i]
                    )
                    ...
                    iou_matrix[pred_id, i] = (
                        -(
                            valid_ba / pred_lines[pred_id].shape[0]
                            + valid_ab / gt_lines[i].shape[0]
                        )
                        / 2
                    )
                    # if iou_matrix[pred_id, i] == 0:
...
if __name__ == "__main__":
    ...
    for i in gt:
        ...
        dilated = LineString(i).buffer(
            1, cap_style=CAP_STYLE.round, join_style=JOIN_STYLE.round
        )
    ...
    for i in pred:
        ...
        dilated = LineString(i).buffer(
            1, cap_style=CAP_STYLE.flat, join_style=JOIN_STYLE.mitre
        )
