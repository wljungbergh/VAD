    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        if not test:
            ...
            assert (pose_record_prev is not None) or (
                pose_record_next is not None
            ), "prev token and next token all empty"
            if pose_record_prev is not None:
                ego_w = (ego_yaw - ego_yaw_prev) / 0.5
                ego_v = np.linalg.norm(ego_pos[:2] - ego_pos_prev[:2]) / 0.5
                ego_vx, ego_vy = ego_v * math.cos(
                    ego_yaw + np.pi / 2
                ), ego_v * math.sin(ego_yaw + np.pi / 2)
            else:
            ...
            info["gt_agent_fut_trajs"] = gt_fut_trajs.reshape(-1, fut_ts * 2).astype(
                np.float32
            )
            info["gt_agent_fut_masks"] = gt_fut_masks.reshape(-1, fut_ts).astype(
                np.float32
            )
